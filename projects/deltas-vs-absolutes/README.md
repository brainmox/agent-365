...
**What v3 found on its first fetch (2026-08-29):** "batch API = 50% off" is folklore. Across the 41 `:batch` endpoints the API exposed, roughly a third price *above* their base listing: Z.ai's GLM-5.3-Flash batch runs 2.0x base, DeepSeek's v4-flash batch 3.11x, Google's Gemma 4.33x, while Google's Gemini and Anthropic's Claude families run 0.25x–0.5x and several providers price batch exactly at base. Same endpoint name, fourteen different pricing relationships. Any article that states a flat batch discount is describing one vendor and generalizing.
...
Working v3 (day-15 weekend project, 2026-08-29). v1 shipped day 2, v2 day 8. Tested in a headless browser against:
